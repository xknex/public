...
install_deps() {
  ...
  "debian"|"ubuntu")
    ...
    if ask_user "Script will execute: '${RED}sudo ${BLUE}apt install -y ${YELLOW}${missing_packages_str}${RESET}'. Proceed?"; then
      echo -e "$LOG Installing missing dependencies"
      if ! sudo apt install -y "${missed_packages[@]}"; then
        echo -e "$ERROR APT terminated with an error."
        exit 1
      else
    ...
    ;;
  *)
    echo -e "$ERROR For now only ${RED}Debian based Linux${RESET} is supported."
    exit 1
    ;;
  esac
}
...
setup_wine() {
  ...
  if ! winetricks --unattended corefonts win10 vkd3d dxvk msxml3 msxml6 gdiplus &>./install_log.log; then
...
install_desktop_entry() {
  ...
  echo "Exec=bash -c "$HOME/.local/bin/photoshop/photoshop.sh"" >>"$path"
